...
/****************************************************************************/
uint_8  s_dnload[BUFFER_LENGTH];         /* DNload Data */
uint_8  parse_flag;                      /* the array is ready to parse */
uint_32 parse_length;                    /* length of parsing array */
uint_8  firmware_block[MAX_BLOCK_SIZE];  /* Firmware block */
uint_8  buffer[MAX_BLOCK_SIZE];          /* Buffer for upload block */
uint_32 firmware_size;                   /* Size of firmware */
uint_32 firmware_offset;                 /* firmware offset */
uint_16 Final_Block;                     /* Final block of new firmware */
extern uint_32 SizeOfFirmware;
extern uint_8 dfu_flash_result;
/**************************************************************************//*!
 *
 ...
 *****************************************************************************/
uint_8 
USB_Class_DFU_Dnload
(
    uint_8 controller_ID,               /* [IN] Controller ID */
    uint_16 wBlockNum,                  /* [IN] The BlockNum of firmware image */
...
)
{
	...
	if ((wBlockNum * MAX_BLOCK_SIZE) > MAX_FIRMWARE_SIZE)
        ...
        if (wBlockNum == 0)
        {
            timeout = ((MAX_FLASH1_ADDRESS -(uint_32) IMAGE_ADDR)/ERASE_SECTOR_SIZE)*10; 
            /* time out for erase flash */
        ...
        {
            USB_Class_DFU_set_bwPoll_TimeOut(1); /* time out for accumulate blocks */
        } /* EndIf */

        for (i=0;i<*size;i++)
        {
            s_dnload[(wBlockNum*MAX_BLOCK_SIZE)%BUFFER_LENGTH + i]=*(*data+i); /* accumulate blocks in s_dnload */
        } /* EndFor */
    } /* EndIf */
    if((((wBlockNum*MAX_BLOCK_SIZE+i) % BUFFER_LENGTH)==0)||(*size == 0)) /* s_dnload full or rest of file */
    {
        /* calculate the length of data in parse array */
        if((wBlockNum*MAX_BLOCK_SIZE+i) % BUFFER_LENGTH) 
             parse_length = (wBlockNum*MAX_BLOCK_SIZE+i) % BUFFER_LENGTH;
        else
             parse_length = BUFFER_LENGTH;
        /* PwPollTimeOut for parse and flash parse array */
        timeout = (uint_32)(parse_length>>4);
        USB_Class_DFU_set_bwPoll_TimeOut(timeout);
        parse_flag = 1;
    } /* EndIf */
    else
    {
    	...
    	USB_Class_DFU_set_bwPoll_TimeOut(15/*50*//*FSL:1,80*/); /* time out for accumulate blocks */
...
USB_Class_DFU_Flashing(void)
{
    ...
    /* the array is ready to parse */  
    if (parse_flag == 1)
    {
        dfu_flash_result = FlashApplication(s_dnload, parse_length);
        parse_flag =0;
    } /* EndIf */
} /* EndBody */
 ...
 *****************************************************************************/
uint_8 
USB_Class_DFU_Upload
(
    uint_8 controller_ID,               /* [IN] Controller ID */
    uint_16 BlockNum,                   /* [IN] Setup packet received */
...
)
{
    ...
    if (BlockNum == 0)
    {
        ...
        if(*(uint_32*)(FIRMWARE_SIZE_ADD) == 0xFFFFFFFF) 
            return  USBERR_INVALID_REQ_TYPE;
        else
            firmware_size = *(uint_32*)(FIRMWARE_SIZE_ADD);
    }
    ...
    for (i=0;i<*size;i++)
    {
        buffer[i] = *(uint_8 *)((uint_8 *)IMAGE_ADDR + firmware_offset + i) ;
    } /* EndFor */
    /* update firmware offset */
    firmware_offset = firmware_offset + wLength;
    *data = buffer;
    return USB_OK;
} /* EndBody */
/**************************************************************************//*!
 ...
 *****************************************************************************/
void 
USB_Class_DFU_Manifest(void)
{
    /* Body */
    if (parse_flag == 1)
    {
        (void)FlashApplication(s_dnload, parse_length);
        parse_flag =0;
    } /* EndIf */
    /* Record the firmware size */
    DisableInterrupts;
    (void)erase_sectors((uint_32*)(FIRMWARE_SIZE_ADD),4);
    (void)Flash_ByteProgram((uint_32*)(FIRMWARE_SIZE_ADD),(uint_32_ptr)(&SizeOfFirmware),4);
    EnableInterrupts;
#ifndef NO_PRINTF_SUPPORT
    printf("\nFlashing Image File Complete!\n");
#endif
} /* EndBody */
/* EOF */
