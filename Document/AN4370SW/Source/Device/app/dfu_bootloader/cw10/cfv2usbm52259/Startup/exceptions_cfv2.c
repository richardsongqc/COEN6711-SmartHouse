 ...
 *
 */
asm void __declspec(register_abi) 
TrapHandler_printf(void) 
{
   HALT
   RTE
}
 ...
 */
void 
derivative_interrupt(unsigned long vector)
{
...
{   /* Interrupt vector table */
   ...
   USB_ISR,           				/* 117 (0x___) USB Interrupt Handler      */
   asm_exception_handler,           /* 118 (0x___) Reserved                   */
#if(MAX_TIMER_OBJECTS > 0)
   Timer_ISR,        				/* 119 (0x___) Prog. Interrupt Timer 0    */
#else
