...
void 
uart_init(int channel, unsigned long systemClockHz, unsigned long baudRate)
{
	 ...
	 */
	ubgs = (uint16)((systemClockKHz)/(baudRate * 32));
...
#else
	ubgs = (uint16)((systemClockHz)/(baudRate * 32));
#endif
...
InitializeUART(UARTBaudRate baudRate)
{
	...
	uart_init(TERMINAL_PORT, BSP_SYSTEM_CLOCK, baudRate);
...
/****************************************************************************/
UARTError 
ReadUARTN(void* bytes, unsigned long limit)
{
	int count;
