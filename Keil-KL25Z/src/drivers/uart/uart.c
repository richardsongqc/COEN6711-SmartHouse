...
void uart0_init (UART0_MemMapPtr uartch, int sysclk, int baud)
{
    ...
    SIM->SOPT2 |= SIM_SOPT2_UART0SRC(1)| SIM_SOPT2_PLLFLLSEL_MASK;    /* UART0 clock source select as MCGFLLCLK/MCGPLLCLK/2*/
...
void uart0_putchar (UART0_MemMapPtr channel, char ch)
{
 ...
 void uart0_putchar1 (UART0_MemMapPtr channel, uint8_t ch)
{
    /* Wait until space is available in the FIFO */
    while(!(UART0_S1_REG(channel) & UART0_S1_TDRE_MASK));
