...
/*----------------------------------------------------------------------------
Initializes the ADC0 for:
	- Single-ended 16 bit conversion 
	- Bus clock as the ADC clock source and configure it for 2MHZ operation
	- Configures for long sample time
	- Software trigger operation mode
	- Compare function disabled
	- DMA request disabled
	- Selects default voltage reference pin pair (External pins VREFH and VREFL)
	- Conversion complete interrupt disabled
	- Input channel 0 selected as ADC input channel
	- Enable hardware averaging, 8 sample averaging. 
Since we are going to use CH0, we have to make sure that this alternative is 
selected by the Signal Multiplexing module. CH0 is located is PTE20 and by 
defualt ADC0 is selected so there is NO need to configure the Signal Multiplexing
module. 
*----------------------------------------------------------------------------*/
//============================================================================
// ****Warning: Any Calibration should be done AFTER calling this function****
//============================================================================
void ADC_Inint(ADC_MemMapPtr ADC_MemPtr){
	en_ADC0();
	ADC_MemPtr->CFG1 = (1<<ADC_CFG1_ADLPC_SHIFT)|ADC_CFG1_ADIV(3)|(1<<ADC_CFG1_ADLSMP_SHIFT)|ADC_CFG1_MODE(3); // LowPower, Adiv=8, LongSample,16bit, BusClk
	ADC_MemPtr->SC2=0x00; //SW trigger is enabled, Compare function is disabled, compare range is disabled, DMA request is disabled,Selects default voltage reference pin pair 
	ADC_MemPtr->SC3= (1<<ADC_SC3_AVGE_SHIFT)|(ADC_SC3_AVGS(1)); // Enable hardware averaging, 8sample averaging, Continuous Conversion disabled 
}
/*----------------------------------------------------------------------------
Read ADC0 function:
	This function returns an UNSIGNED 32 bit value. Before calling this function 
you need to call the ADC_Inint function to make sure that the ADC module is 
initialized properly.
Each time that the function is called, it will check the COCO bit to make sure
that there is no conversion in progress. (Refer to page 501 on the datasheet for
more information)
*----------------------------------------------------------------------------*/
uint32 Read_ADC(ADC_MemMapPtr ADC_MemPtr){
		uint32 AdcOut=0;
		AdcOut=(uint32)ADC_MemPtr->R[A];
		while (((((ADC_MemPtr->SC1[A]) & ADC_SC1_COCO_MASK )) == 0x80u)){
		}
		ADC_MemPtr->SC1[A]=ADC_SC1_ADCH(0); // Interupt disabled, single ended conversion, Input Channel 0 is selected
		while (!((((ADC_MemPtr->SC1[A]) & ADC_SC1_COCO_MASK )) == 0x80u)){
		}
		AdcOut=(uint32)ADC_MemPtr->R[A];
		ADC_MemPtr->SC1[A]=ADC_SC1_ADCH(31); // Disable ADC
		return AdcOut;
}
/*----------------------------------------------------------------------------
ADC0 disable function:
	This function will disable the ADC0 module by disabling the ADC0 Clock Gating
in System Clock Gating Control Register 6. Refer to Page 207 for more information.
*----------------------------------------------------------------------------*/
void dis_ADC0(void){
	SIM->SCGC6 = (~(1<<SIM_SCGC6_ADC0_SHIFT)) & (SIM->SCGC6);
}
/*----------------------------------------------------------------------------
ADC0 enable function:
	This function will enable the ADC0 module by enabling the ADC0 Clock Gating
in System Clock Gating Control Register 6. Refer to Page 207 for more information.
*----------------------------------------------------------------------------*/
void en_ADC0(void){
		SIM->SCGC6 = (1<<SIM_SCGC6_ADC0_SHIFT) | (SIM->SCGC6);
